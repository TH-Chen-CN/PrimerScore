...
def build_pair_source_profiles(
    ...
    # Defensive normalization for cross-machine / cross-source robustness
    if not isinstance(target_expr_by_tissue, dict):
        target_expr_by_tissue = {}
    if not isinstance(near_expr_by_tissue, dict):
        near_expr_by_tissue = {}
    if not isinstance(far_expr_by_tissue, dict):
        far_expr_by_tissue = {}
    if not isinstance(target_vs_near_ratio, dict):
        target_vs_near_ratio = {}
    if not isinstance(target_vs_far_ratio, dict):
        target_vs_far_ratio = {}

    dominant_target_tissues = _normalize_str_list(
        e_eval.get("dominant_target_tissues", [])
    )
    ...
    for tissue in tissues:
        ...
        is_dominant = tissue in dominant_target_tissues
        ...
        score_detail = _compute_expression_score_detail(
            ...
            is_dominant_tissue=is_dominant,
        )
        ...
        profiles.append({
            ...
            "is_dominant_target_tissue": is_dominant,
...
def rank_combined(
    ...
    for item in pair_source_profiles:
        ...
        thermo_label = _safe_str(thermo.get("label", "unknown")) or "unknown"
        specificity_label = _safe_str(specificity.get("label", "unknown")) or "unknown"
        ...
        final_warnings = [
            *_normalize_str_list(item.get("source_warnings", [])),
            *_normalize_str_list(thermo.get("warnings", [])),
            *_normalize_str_list(specificity.get("warnings", [])),
        ]

        final_tags = [
            *_normalize_str_list(item.get("source_tags", [])),
            thermo_label,
            specificity_label,
        ]
        ...
        rows.append({
            ...
            "source_tags": _normalize_str_list(item.get("source_tags", [])),
            "source_warnings": _normalize_str_list(item.get("source_warnings", [])),
